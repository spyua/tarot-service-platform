...
export class SharingService {
   ...
   * 生成分享內容
   */
  public async generateShareContent(
    ...
    const question =
      'question' in reading ? (reading as any).question : undefined;

    // 基本分享文字
    let text = '我的塔羅占卜結果：\n';

    // 根據隱私設定添加牌名
    if (this.privacyManager.canShareCardNames()) {
      const cardNames = cards
        .map(c => `${c.card.name}${c.isReversed ? '(逆位)' : ''}`)
        .join('、');
      text += `抽到的牌：${cardNames}\n`;
    }
    ...
    if (
      ...
      question &&
      this.privacyManager.canShareQuestion()
    ) {
      text += `我的問題：${question}\n`;
    }
    ...
    if (
      ...
      this.privacyManager.canShareInterpretation()
    ) {
    ...
    if (options.includeImage && this.privacyManager.canShareCardNames()) {
      try {
        imageUrl = await this.generateShareImage(reading);
      } catch (error) {
   ...
   * 生成分享圖片
   */
  private async generateShareImage(reading: ReadingResult): Promise<string> {
    try {
      // 使用 imageUtils 中的方法生成圖片
      const title =
        reading.type === 'daily' ? '我的每日塔羅指引' : '我的塔羅占卜結果';

      return await generateShareImage(reading.cards, title);
    } catch (error) {
      console.error('生成分享圖片失敗:', error);
      throw error;
    }
